...
function createTaskEventHandler() {
  ...
  const hasInvalidInput = Object.values(inputDOMSelectors).some(
    (input) => input.value === ""
  );
  if (hasInvalidInput) {
    return;
  ...
  fetch(BASE_URL, headers).then(loadBoardEventHandler).catch(console.error);
...
async function moveTaskHandler(e) {
  ...
  const headers = {
    method: "PATCH",
    body: JSON.stringify({
      status: textContentToStatusMap[button.textContent],
    }),
  };
...
function clearAllSections() {
  Object.values(statusToContainerMap).forEach((section) => {
    section.innerHTML = "";
  });
}
